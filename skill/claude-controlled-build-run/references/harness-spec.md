> **PORTING NOTE (see `PORTING.md`):** the "canonical" snippets below were
> written for a Python/uv host; the shipped worked examples under `examples/`
> come from a TS/pnpm host. In an ESTABLISHED repo the LIVE files are always
> canonical over any snippet here — read the snippets as the spec of what each
> piece must do, and express them in your host's toolchain.

# Harness spec — verify or install each piece
...
**Source of truth:** in an established repo, the *live* files are canonical —
...
stripped of comments. A live file will usually be *richer* than the version here
...
*behavior* is actually wrong or missing. Never overwrite a working hook just to
...
- **Bricked / blocking everything?** If every Bash/Write/Edit returns `Probity:
  Cannot find module '<x>'`, the config failed to *load* and Probity fails **closed**
...
project-prefixed imports (`from proj_X`), forbid jargon imports (`engine`,
...
Max ~2 fix rounds per finding-surface (a new defect resets the counter), then escalate or decline by judgment with recorded reasoning. Closing after a fix is the step most easily
  ...
  - repo: local
    hooks:
      ...
      - id: pytest
        ...
        always_run: true
      - id: roborev-clean
        name: roborev (no open reviews on this branch)
        entry: ./scripts/roborev-clean-gate.sh
        language: system
        pass_filenames: false
        always_run: true
```

The `roborev-clean` hook is the close-every-review law made mechanical: it
refuses the next commit while this branch has any FAIL/pending review, and
auto-closes clean PASS reviews with a stamped note (so clean work pays no
toll). It needs the tracked script `scripts/roborev-clean-gate.sh` — verify
both ride together: `grep -q roborev-clean .pre-commit-config.yaml && test -x
scripts/roborev-clean-gate.sh && echo armed || echo MISSING-GATE`. A branch
cut before this gate landed will report `MISSING-GATE` even with the harness
otherwise armed — see the inheritance note below.
...
the other three. When *adopting* the formatter on a repo that has pre-existing
...
- **What it injects (COMPOSED, role-aware).** The payload is composed from the
  ...
  - when a `task_plan.md` exists (a build is in progress): the core law files
    ...
    policy/strand/reviews/judgment always, plus fleet+captain modes for an
...
- **Verify:** run the script and count the anchored section headers:
  ...
  fleet/captain, and an oversized plan trio must still emit valid JSON (the
...
nothing (the symptom: after a compaction the rules and plan are *not* back in
context, and a fresh-feeling agent has to re-read the skill by hand). The event
that actually re-injects is **`SessionStart`**, whose `additionalContext` *is*
...
is a *second* `SessionStart` hook with no matcher, so it runs on every start; the
...
PORTING block: set it to `1` only in such a repo (the origin host was one, and paired it
...
kept because it documents *why* the permission layer cannot gate an unattended builder —
...
at the pane, a prompt is a silent hang. That includes *read-only* commands you'd
...
- **Verify:** `test -f .claude/settings.local.json && echo ok`, then a live probe
  ...
  *permitted* (see the prove-YES step in `references/core/strand.md`).
...
```json
{
  ...
  "permissions": {
    "allow": [
      ...
      "Bash(uv --version:*)", "Bash(uv run:*)", "Bash(uv sync:*)",
      "Bash(roborev:*)",
...
NOT loosen the real fences: Probity (folder-scoped) and the roborev-clean gate
(branch-scoped) are unchanged — it only suppresses the redundant self-isolation step.
...
repo** (canon is trusted). If a `--bg` builder is nonetheless stuck (`cbr.sh status`
...
shares the `SessionStart` event with the session-sweep (it is a *second*
  ...
  "hooks": {
    "PreToolUse": [
      ...
      { "matcher": "AskUserQuestion",
        "hooks": [ { "type": "command", "command": "bash .claude/hooks/no-interactive-ask.sh", "timeout": 10 } ] }
    ],
    "PostToolUse": [
      { "matcher": "Bash",
        "hooks": [ { "type": "command", "command": "bash .claude/hooks/roborev-gate.sh",
          "asyncRewake": true, "timeout": 300,
          "rewakeMessage": "RoboRev flagged the last commit (FAIL). Handle it, then update the review so the open-FAIL list stays honest: if you fixed it, close it (roborev respond <job> -m '...' && roborev close <job>); if it's wrong or intentional, close-with-reason the same way; leave it open ONLY if it is deliberately deferred or needs the operator. Max 2 fix rounds per finding-surface (a genuinely NEW defect resets the counter), then stop fixing: escalate, or decline by judgment with recorded reasoning surfaced to the human. Finding follows:",
          "rewakeSummary": "RoboRev FAIL on last commit" } ] }
    ],
    "SessionStart": [
      { "hooks": [ { "type": "command", "command": "bash .claude/hooks/roborev-session-sweep.sh", "timeout": 30 } ] },
      { "matcher": "compact",
        "hooks": [ { "type": "command", "command": "bash .claude/hooks/post-compact-reground.sh", "timeout": 15 } ] }
    ]
  }
}
```
