# Codex agent-harness specification
...
One nondeterministic judge flake may be retried once. Two consecutive unblocked production writes mean `CONTROL-PLANE-BROKEN.marker`: stop the build.
...
The repository's live `.pre-commit-config.yaml` defines the facts. It must include its real format, lint, type, test, and secret commands (per-commit reviews are advisory; the blocking review check rides the merge path). All must be verify-only: a gate reports and blocks rather than silently rewriting files.
...
complexity-ceiling row — in projects that wire the optional ceiling (a
per-project knob, not core law): there it is a deterministic pre-commit gate, so
an agent that has drifted past the rule meets it as a blocked commit, which is
the most expensive moment to first read it. It is resolved the same two ways this
...
It states that boot and control-plane setup already happened, so the resumed agent
...
Set the source agent harness's effective compact trigger in project config:
...
- `CONTROL-PLANE-BROKEN.marker`: terminal guard failure.
- `DONE-<branch>.marker`: committed final completion signal (per-branch name).
