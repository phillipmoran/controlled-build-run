...
`CONTROL-PLANE-BROKEN.marker` if either is not proven. Use watched-fail TDD for every guarded behavior,
...
recorded Codex thread. Do not rerun control-plane boot after compaction. Finish by
committing your branch's `DONE-<branch>.marker` — replace this placeholder with
the EXACT filename before dispatch (strand-lib `cbr_done_marker_name` derives it;
launch refuses a prompt that does not name it) containing the final commit, verification evidence,
and review state. If safe progress is impossible, write `NEEDS-HUMAN.md` or
`CONTROL-PLANE-BROKEN.marker` with exact evidence and stop.
