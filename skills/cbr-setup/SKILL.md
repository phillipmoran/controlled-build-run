...
an armed repo serves Claude Code and Codex alike, and teammates without this
plugin still get every repo-level gate.

## 2. Run the real installer

Follow `controlled-build-run/SETUP.md` (the Claude Code entry point) end to
end. That document owns the details. Honor these rules while you do:
...
- **Write repo-level hooks, not just plugin hooks.** Install the settings
  ...
  `.git/hooks` per SETUP.md, so the guard does not depend on any one
  person's plugin.
